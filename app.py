...
def save_upload(file):
    ...
    safe = uid + "_" + file.filename.replace(" ","_")
...
def status(): return jsonify({"status":"OfficePDFEditor API running","version":"3.0"})
...
def compress():
    ...
    file=request.files["file"]; quality=request.form.get("quality","balanced")
    try:
        import pypdf; from PIL import Image as PILImage
        input_path,fname=save_upload(file); orig_size=os.path.getsize(input_path)
        reader=pypdf.PdfReader(input_path); writer=pypdf.PdfWriter()
        img_quality={"low":25,"balanced":50,"high":72}.get(quality,50)
        ...
        if out_size>=orig_size:
            import shutil; shutil.copy(input_path,output); out_size=orig_size
        user=get_user_from_token(get_token(request))
        if user: save_history(user["id"],"Compress PDF",file.filename,orig_size,out_size)
        saved=orig_size-out_size; pct=round((saved/orig_size)*100,1) if orig_size>0 else 0
...
def merge():
    ...
    try:
        import pypdf; writer=pypdf.PdfWriter(); total_size=0
        for file in files:
            path,_=save_upload(file); total_size+=os.path.getsize(path)
...
def split():
    if "file" not in request.files: return jsonify({"error":"File upload karein"}),400
    file=request.files["file"]; split_type=request.form.get("split_type","all"); page_range=request.form.get("page_range","").strip()
    try:
        import pypdf,zipfile; input_path,fname=save_upload(file); reader=pypdf.PdfReader(input_path); total=len(reader.pages)
        ...
        with zipfile.ZipFile(zip_output,"w") as zf:
            if split_type=="range" and page_range:
                ...
                for part in page_range.split(","):
                    part=part.strip()
                    if "-" in part:
                        a,b=part.split("-"); pages.update(range(int(a)-1,min(int(b),total)))
                    elif part: pages.add(int(part)-1)
                writer=pypdf.PdfWriter()
                for p in sorted(pages):
                    if 0<=p<total: writer.add_page(reader.pages[p])
                buf=io.BytesIO(); writer.write(buf); zf.writestr("split_pages.pdf",buf.getvalue())
            else:
                for i,page in enumerate(reader.pages):
                    writer=pypdf.PdfWriter(); writer.add_page(page)
                    buf=io.BytesIO(); writer.write(buf); zf.writestr(f"page_{i+1}.pdf",buf.getvalue())
        user=get_user_from_token(get_token(request))
...
def rotate():
    if "file" not in request.files: return jsonify({"error":"File upload karein"}),400
    file=request.files["file"]; angle=int(request.form.get("angle",90))
    try:
        import pypdf; input_path,fname=save_upload(file); reader=pypdf.PdfReader(input_path); writer=pypdf.PdfWriter()
        for page in reader.pages: page.rotate(angle); writer.add_page(page)
...
def delete_pages():
    if "file" not in request.files: return jsonify({"error":"File upload karein"}),400
    file=request.files["file"]; pages_str=request.form.get("pages","")
    try:
        import pypdf; input_path,fname=save_upload(file); reader=pypdf.PdfReader(input_path)
        to_del=set()
...
def jpg_to_pdf():
    ...
    try:
        ...
        for f in files:
            path,_=save_upload(f); images.append(PILImage.open(path).convert("RGB"))
        output=out_path("_converted.pdf")
        if len(images)==1: images[0].save(output,"PDF",resolution=100)
        else: images[0].save(output,"PDF",save_all=True,append_images=images[1:],resolution=100)
        return jsonify({"download_id":os.path.basename(output)})
    except Exception as e: return jsonify({"error":str(e)}),500
...
def pdf_to_jpg():
    ...
    try:
        import pypdf,zipfile; from PIL import Image as PILImage
        input_path,fname=save_upload(file); reader=pypdf.PdfReader(input_path)
        zip_output=out_path("_images.zip"); count=0
        with zipfile.ZipFile(zip_output,"w") as zf:
                    ...
                    for name in list(xobj.keys()):
                        try:
                            ...
                            if hasattr(obj,'get') and obj.get("/Subtype")=="/Image":
                                ...
                                zf.writestr(f"page{page_num+1}_img{count+1}.jpg",buf.getvalue()); count+=1
                        except: pass
                except: pass
        if count==0: return jsonify({"error":"PDF mein images nahi mili. Yeh text-based PDF ho sakta hai."}),400
        return jsonify({"download_id":os.path.basename(zip_output)})
    except Exception as e: return jsonify({"error":str(e)}),500
...
def watermark():
    if "file" not in request.files: return jsonify({"error":"File upload karein"}),400
    file=request.files["file"]; text=request.form.get("text","CONFIDENTIAL")
    try:
        import pypdf; from reportlab.pdfgen import canvas; from reportlab.lib.pagesizes import A4
        input_path,fname=save_upload(file); reader=pypdf.PdfReader(input_path)
        wm_buf=io.BytesIO(); c=canvas.Canvas(wm_buf,pagesize=A4)
        c.setFont("Helvetica-Bold",52); c.setFillColorRGB(0.75,0.75,0.75,alpha=0.25)
        c.saveState(); c.translate(A4[0]/2,A4[1]/2); c.rotate(45); c.drawCentredString(0,0,text); c.restoreState(); c.save()
        wm_buf.seek(0); wm_page=pypdf.PdfReader(wm_buf).pages[0]
        writer=pypdf.PdfWriter()
...
def add_page_numbers():
    ...
    file=request.files["file"]
    try:
        import pypdf; from reportlab.pdfgen import canvas
        input_path,fname=save_upload(file); reader=pypdf.PdfReader(input_path); writer=pypdf.PdfWriter()
        for i,page in enumerate(reader.pages):
            ...
            overlay=pypdf.PdfReader(buf); page.merge_page(overlay.pages[0]); writer.add_page(page)
...
def count_words():
    ...
    file=request.files["file"]
    try:
        import pdfplumber; input_path,fname=save_upload(file); text=""
...
def pdf_info():
    ...
    try:
        ...
        return jsonify({"pages":len(reader.pages),"title":meta.get("/Title","N/A"),"author":meta.get("/Author","N/A"),
                        "encrypted":reader.is_encrypted,"file_size":os.path.getsize(input_path)})
    except Exception as e: return jsonify({"error":str(e)}),500

@app.route("/api/lock", methods=["POST"])
def lock_pdf():
    if "file" not in request.files: return jsonify({"error":"File upload karein"}),400
    file=request.files["file"]; password=request.form.get("password","")
    if not password: return jsonify({"error":"Password enter karein"}),400
    try:
        import pypdf; input_path,fname=save_upload(file); reader=pypdf.PdfReader(input_path); writer=pypdf.PdfWriter()
        for page in reader.pages: writer.add_page(page)
        writer.encrypt(password); output=out_path("_locked.pdf")
        with open(output,"wb") as f: writer.write(f)
        return jsonify({"download_id":os.path.basename(output)})
    except Exception as e: return jsonify({"error":str(e)}),500
...
def unlock_pdf():
    ...
    try:
        ...
        if reader.is_encrypted:
            if not reader.decrypt(password): return jsonify({"error":"Password galat hai"}),400
        writer=pypdf.PdfWriter()
        for page in reader.pages: writer.add_page(page)
...
def download(file_id):
    if ".." in file_id or "/" in file_id: return jsonify({"error":"Invalid"}),400
    path=os.path.join(OUTPUT_FOLDER,file_id)
    if not os.path.exists(path): return jsonify({"error":"File nahi mili ya expire ho gayi"}),404
